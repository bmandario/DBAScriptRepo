...
/****** Object:  StoredProcedure [dbo].[Login_Audit_Insert]    Script Date: 1/7/2025 9:03:11 AM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[Login_Audit_Insert]
...
------SYSTEM/SA--------
insert into Login_Audit (DOMAIN, SERVER,LOGIN_ID,LOGON_ID_TYPE,Privilege,Date_Executed) 
SELECT DEFAULT_DOMAIN(), @@SERVERNAME, a.name as logins ,a.type_desc,  c.name, getdate()
...
---DB User Permissions---
insert into Login_Audit  (DOMAIN, SERVER,DATABASES,Privilege,LOGIN_ID,LOGON_ID_TYPE,Date_Executed) 
exec master.dbo.sp_MSforeachdb 'use [?] 
SELECT DEFAULT_DOMAIN(), @@SERVERNAME, db_name(), c.name,a.name, a.type_desc, getdate()
...
WHERE a.name <> ''dbo''and c.is_fixed_role=1 and c.name in (''db_datawriter'',''db_owner'',''db_securityadmin'',''db_accessadmin'',''db_securityadmin'',''db_ddladmin'') 
