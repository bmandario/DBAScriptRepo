...
# 2. Log results
#
$ErrorLog = 'D:\Scripts\PS\SQL Job Email Validation\error.txt'

$Connection = New-Object System.Data.SQLClient.SQLConnection
$Connection.ConnectionString = "server=USCHWSQL1383;database=master;trusted_connection=true;"
$Connection.Open()
...
$Query = Get-Content 'D:\Scripts\SQL\Maintenance_Job_Info_Script.txt'
...
foreach ($Row in $Result) {
  ...
  try {
    ...
    if ($DataSet2.Tables[0].Rows.Count -gt 0) {
      $SqlBulkCopy = New-Object ('Data.SqlClient.SqlBulkCopy') "Server=USCHWSQL1383;Database=DBA_WORK;Trusted_Connection=True"
