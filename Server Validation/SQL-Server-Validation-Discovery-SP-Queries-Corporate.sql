...
FROM [DISCOVERY].[dbo].[v_WinViewALL] v_WVA
...
FROM [DISCOVERY].[dbo].[v_WinViewDisk] v_WVD
...
FROM [DISCOVERY].[dbo].[v_InstanceProperties_All] v_IP_A
...
-- 3rd party software
INSERT INTO [ServerValidation].[des].[InstalledSoftware]  ([ValidationID],[ComputerName],[ProductName])
SELECT @CurrentID, sccm_V_R_S.[Name0] AS ComputerName, v_GS_S.[DisplayName0] AS ProductName
FROM [DISCOVERY].[dbo].[sccm_V_R_System] sccm_V_R_S
LEFT JOIN [USCHIZWSCCM1001].[CM_GP1].[dbo].[v_GS_SERVICE] v_GS_S ON sccm_V_R_S.[resourceID] = v_GS_S.[resourceID]
WHERE v_GS_S.[DisplayName0] LIKE '%control-m%' AND sccm_V_R_S.[Name0] LIKE @SourceServer + '%'
...
FROM [DISCOVERY].[dbo].[sccm_v_GS_Service] sccm_v_GS_S
...
FROM [DISCOVERY].[dbo].[sccm_V_R_System] sccm_V_R_S
LEFT JOIN [USCHIZWSCCM1001].[CM_GP1].[dbo].[v_GS_SERVICE] v_GS_S ON sccm_V_R_S.[resourceID] = v_GS_S.[resourceID]
WHERE v_GS_S.[DisplayName0] LIKE '%control-m%' AND sccm_V_R_S.[Name0] LIKE @DestinationServer + '%'
